            ...
            <div class="layout-page">
                ...
                <div class="content-wrapper">
                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                        <h4 class="py-3 mb-4"><span class="text-muted fw-light">Tables /</span> Data Kegiatan</h4>

                        {{-- 🔍 Fitur Pencarian & Pengurutan --}}
<form method="GET" action="{{ route('admin.kegiatan.index') }}" class="d-flex mb-3 align-items-center gap-2">
    <input 
        type="text" 
        name="search" 
        class="form-control" 
    ...
    <select name="sort" class="form-select" style="max-width: 200px;">
        <option value="kode_kegiatan" {{ request('sort') == 'kode_kegiatan' ? 'selected' : '' }}>Urutkan: Kode</option>
        <option value="nama_kegiatan" {{ request('sort') == 'nama_kegiatan' ? 'selected' : '' }}>Urutkan: Nama</option>
        <option value="tanggal_mulai_kegiatan" {{ request('sort') == 'tanggal_mulai_kegiatan' ? 'selected' : '' }}>Urutkan: Tanggal Mulai</option>
    </select>
                                ...
                                <table class="table table-striped">
                                    ...
                                    <tbody class="table-border-bottom-0">
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ \Carbon\Carbon::parse($item->tanggal_mulai_kegiatan)->format('d/m/Y') }}</td>
                                            <td>{{ \Carbon\Carbon::parse($item->tanggal_selesai_kegiatan)->format('d/m/Y') }}</td>
                                            <td>{{ $item->lokasi_kegiatan }}</td>
                                            <td>{{ $item->jam_mulai_kegiatan }}</td>
                                            <td>{{ $item->jam_selesai_kegiatan }}</td>
                                            <td>{{ \Illuminate\Support\Str::limit($item->deskripsi_kegiatan, 40) }}</td>
                                            <td>{{ $item->kuota_kegiatan }}</td>
                                            <td>
                                            ...
                                            <td>
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu">
                                                        <a class="dropdown-item" href="{{ route('admin.kegiatan.edit', $item->id) }}"><i class="bx bx-edit-alt me-1"></i> Edit</a>
                                                        <form action="{{ route('admin.kegiatan.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this item?');">
                                                            @csrf
                                                            @method('DELETE')
