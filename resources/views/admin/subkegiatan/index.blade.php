...
<head>
    ...
    <title>Dashboard Sub Kegiatan</title>
            ...
            <div class="layout-page">
                ...
                <div class="content-wrapper">
                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                        <h4 class="py-3 mb-4"><span class="text-muted fw-light">Tables /</span> Data Sub Kegiatan</h4>

                        {{-- 🔍 Fitur Pencarian & Pengurutan --}}
                        <form method="GET" action="{{ route('admin.subKegiatan.index') }}" class="d-flex mb-3 align-items-center gap-2">
                            <select name="filter_sub" class="form-select" style="max-width: 250px;">
                                <option value="">-- Semua Sub Kegiatan --</option>
                                @foreach ($allsubKegiatan as $sub)
                                    <option value="{{ $sub->id }}" {{ request('filt
                                    er_sub') == $sub->id ? 'selected' : '' }}>
                            ...
                            {{-- 🔽 Dropdown Filter Sub Kegiatan --}}
                            <select name="filter_sub" class="form-select" style="max-width: 250px;">
                                <option value="">-- Semua Sub Kegiatan --</option>
                                @foreach ($allsubKegiatan as $sub)
                                    <option value="{{ $sub->id }}" {{ request('filter_sub') == $sub->id ? 'selected' : '' }}>
                                        {{ $sub->nama_subKegiatan }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select name="sort" class="form-select" style="max-width: 200px;">
                                <option value="kode_subKegiatan" {{ request('sort') == 'kode_subKegiatan' ? 'selected' : '' }}>Urutkan: Kode</option>
                                <option value="nama_subKegiatan" {{ request('sort') == 'nama_subKegiatan' ? 'selected' : '' }}>Urutkan: Nama</option>
                            </select>

                            <select name="direction" class="form-select" style="max-width: 150px;">
                        ...
                        <a href="{{ route('admin.subKegiatan.create') }}" class="btn btn-primary">Tambah</a>

                        <!-- Striped Rows -->
                        <div class="card">
                            <h5 class="card-header">Data Sub Kegiatan</h5>
                            <div class="table-responsive text-nowrap">
                                <table class="table table-striped">
                                    ...
                                    <tbody class="table-border-bottom-0">
                                        ...
                                        <tr>
                                            <td>{{ $subKegiatan->firstItem() + $loop->index }}</td>
                                            <td>{{ $item->kode_subKegiatan }}</td>
                                            <td>{{ $item->nama_subKegiatan }}</td>
                                            <td>{{ $item->kegiatan->nama_kegiatan ?? '-' }}</td>
                                            <td>{{ \Carbon\Carbon::parse($item->tanggal_mulai_sub)->format('d/m/Y') }} - 
    {{ \Carbon\Carbon::parse($item->tanggal_selesai_sub)->format('d/m/Y') }}
</td>
<td>{{ $item->jam_mulai_sub ?? '-' }}</td>
<td>{{ $item->jam_selesai_sub ?? '-' }}</td>
<td>
    <div class="dropdown">
        ...
        <div class="dropdown-menu">
            <a class="dropdown-item" href="{{ route('admin.subKegiatan.edit', $item->id) }}">
                <i class="bx bx-edit-alt me-1"></i> Edit
            </a>
            <form action="{{ route('admin.subKegiatan.destroy', $item->id) }}" method="POST" 
