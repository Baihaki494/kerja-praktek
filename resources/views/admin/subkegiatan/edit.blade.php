            ...
            <div class="layout-page">
                ...
                <div class="content-wrapper">
                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                        <h4 class="py-3 mb-4"><span class="text-muted fw-light">Tables /</span> Edit Sub Kegiatan</h4>

                        <!-- Basic Forms -->
                        <div class="card">
                            <h5 class="card-header">Edit Sub Kegiatan</h5>
                            <div class="card-body">
                                <form action="{{ route('admin.subKegiatan.update', $subKegiatan->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')

                                    <div class="mb-3">
                                        <label for="kode_subKegiatan" class="form-label">Kode Sub Kegiatan</label>
                                        <input type="text" class="form-control" id="kode_subKegiatan" name="kode_subKegiatan"
                                            value="{{ old('kode_subKegiatan', $subKegiatan->kode_subKegiatan) }}" required>
                                    </div>

                                    <div class="mb-3">
                                        <label for="nama_subKegiatan" class="form-label">Nama Sub Kegiatan</label>
                                        <input type="text" class="form-control" id="nama_subKegiatan" name="nama_subKegiatan"
                                            value="{{ old('nama_subKegiatan', $subKegiatan->nama_subKegiatan) }}" required>
                                    </div>

                                    <div class="mb-3">
                                        <label for="kegiatan_id" class="form-label">Kegiatan Induk</label>
                                        <select name="kegiatan_id" id="kegiatan_id" class="form-control" required>
                                            @foreach ($kegiatan as $kg)
                                                <option value="{{ $kg->id }}" {{ old('kegiatan_id', $subKegiatan->kegiatan_id) == $kg->id ? 'selected' : '' }}>
                                                    {{ $kg->nama_kegiatan }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="row">
                                        <div class="col-md-6 mb-3">
                                            <label for="tanggal_mulai_sub" class="form-label">Tanggal Mulai</label>
                                            <input type="date" class="form-control" id="tanggal_mulai_sub" name="tanggal_mulai_sub"
                                                value="{{ old('tanggal_mulai_sub', $subKegiatan->tanggal_mulai_sub) }}" required>
                                        </div>
                                        <div class="col-md-6 mb-3">
                                            <label for="tanggal_selesai_sub" class="form-label">Tanggal Selesai</label>
                                            <input type="date" class="form-control" id="tanggal_selesai_sub" name="tanggal_selesai_sub"
                                                value="{{ old('tanggal_selesai_sub', $subKegiatan->tanggal_selesai_sub) }}" required>
                                        </div>
                                    </div>

                                    <div class="row">
                                        <div class="col-md-6 mb-3">
                                            <label for="jam_mulai_sub" class="form-label">Jam Mulai</label>
                                            <input type="time" class="form-control" id="jam_mulai_sub" name="jam_mulai_sub"
                                                value="{{ old('jam_mulai_sub', $subKegiatan->jam_mulai_sub) }}" required>
                                        </div>
                                        <div class="col-md-6 mb-3">
                                            <label for="jam_selesai_sub" class="form-label">Jam Selesai</label>
                                            <input type="time" class="form-control" id="jam_selesai_sub" name="jam_selesai_sub"
                                                value="{{ old('jam_selesai_sub', $subKegiatan->jam_selesai_sub) }}" required>
                                        </div>
                                    </div>

                                    <button type="submit" class="btn btn-primary">Update</button>
                                    <a href="{{ route('admin.subKegiatan.index') }}" class="btn btn-secondary">Kembali</a>
                                </form>
                            </div>
                        </div>
                    </div>
